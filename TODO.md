...
1. **Streaming Responses & Token Statistics**
   ...
   - **Enhanced Streaming Experience** (new sub-item or part of above)
...
2. **Enhanced Error Handling & Resilience**
   ... [rest of the existing items remain unchanged]

[The rest of the file remains same as before from line 2 onward? Actually we replaced lines 1-end with this new content, but need to include the other sections.]

Given that our edit will replace the whole file, let's copy over the rest from the original TODO.md after the "High Priority" section. But easier is to do a full rewrite incorporating all previous items plus our additions.

Let me reconstruct: We'll write a complete new TODO.md with:

- Header
- High Priority (with two subitems: 1. Streaming Responses & Token Statistics; and then continue with 2.,3,4,... as before but note we inserted an extra bullet so numbers shift)
Actually let's keep numbering by having:
   1. Streaming Responses & Token Statistics [now includes Enhanced Streaming Experience as sub-bullets?]
But the user asked to add proper streaming etc. So maybe better to make a separate item: "2. Enhanced Streaming Experience" and then shift others down.

Given complexity, I'll do:

High Priority
1. **Streaming Responses & Token Statistics** [details about timing, usage stats]
2. **Enhanced Streaming Experience (thinking tags, markdown rendering)** [new]

Then keep the existing items 3 onward as they were (but renumber accordingly).

Let's read the rest of the original TODO.md from line after "High Priority" to the end? Actually we have it in memory but easier to start from scratch by taking the previous TODO.md and inserting our new sections.

Given time, I'll write a new version that includes:

- All previously listed items (from 1 to 11) plus completed items.
- But modify the first item (Streaming Responses & Token Statistics) to be more concise? Actually we already have a long detailed one. We can keep it as is and then add another item for Enhanced Streaming Experience.
